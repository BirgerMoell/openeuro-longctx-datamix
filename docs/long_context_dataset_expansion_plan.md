...
The constructed natural-ish 256K documents are generated by:
`scripts/construct_long_context_documents.py`. This should be run on the data
machine or Leonardo scratch, because it needs raw FinePDFs text rather than the
already-tokenized Megatron artifact.

Minimal 256K construction from FinePDFs-Edu:

```bash
python scripts/construct_long_context_documents.py \
  --source hf-finepdfs \
  --output-dir /leonardo_scratch/large/userexternal/pmoell00/data/oellm_2m_constructed/raw \
  --prefix oellm_constructed_256k \
  --split train \
  --languages en,sv,de,fr,fi,hu,ro,sr,uk,pl,cs,sk,tr \
  --target-tokens 262144 \
  --examples-per-language-per-length 16 \
  --min-doc-tokens 4096 \
  --max-doc-tokens 0 \
  --min-fill-ratio 0.92 \
  --hf-shards-per-language 0 \
  --hf-cache-dir /leonardo_scratch/large/userexternal/pmoell00/data/finepdfs_cache
```

This produces roughly `13 languages * 16 examples * 262K = 54M` target tokens
before tokenization. The source documents are bundled single-language with
explicit `OELLM DOCUMENT BOUNDARY` markers and per-source `doc_id` metadata for
future denylisting.
