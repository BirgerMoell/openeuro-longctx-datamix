...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 8, "OpenEuroLLM Long-Context Training - Job 18536300", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    def kv_row(self, key, value, alt=False):
        ...
        self.set_font("Helvetica", "B", 9)
        self.cell(65, 6.5, key, fill=True)
        self.set_font("Helvetica", "", 9)
        self.cell(0, 6.5, value, fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
pdf.cell(0, 12, "YaRN Multilingual Training Report", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
pdf.set_font("Helvetica", "", 11)
pdf.set_text_color(80, 80, 80)
pdf.cell(0, 7, "Full Training Run - SLURM Job 18536300", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
pdf.set_font("Helvetica", "", 9)
pdf.cell(0, 6, "OpenEuroLLM Long-Context Pipeline  |  LUMI Supercomputer (CSC)  |  2026-05-10", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
pdf.body_text(
    "999 iterations logged. Table shows every 100th iteration plus first and last. "
    "Throughput measured on rank 255 (last pipeline stage)."
)
...
for i, (it, loss, gn, tf, note) in enumerate(LOSS_TABLE):
    ...
    pdf.set_font("Helvetica", "B" if it in (2, 1000) else "", 8.5)
    ...
    pdf.set_font("Helvetica", "I" if note else "", 8)
...
pdf.cell(0, 7, "  Final validation loss: 3.5679  |  Validation PPL: 35.44", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
pdf.cell(0, 7, "  Final test loss:       3.4302  |  Test PPL:       30.88", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)

# ── Analysis ──────────────────────────────────────────────────────────────────
pdf.section_title("5. Analysis")
pdf.body_text(
    "The model began training at loss 12.22 - significantly above the random baseline of "
    "ln(262144) ~= 12.48. This elevated starting loss is expected: the base checkpoint was "
    "pre-trained at 2048-token context, and YaRN's modified rotary position embeddings "
    ...
    "an hour of training. This rapid early descent reflects the model quickly learning the "
    "structure of YaRN-scaled positions. Gradient norms peak at 73 in the first iterations "
    "then settle to 1-2 by iter 200, confirming stable convergence.\n\n"
    ...
    "Throughput was rock-solid at 40.5-40.6 TFLOP/s/GPU across all 256 GPUs for the "
    "entire 9-hour run (~513 tok/s/GPU). The only slow iteration was iter 2 (16.1 TFLOP/s) "
    "due to dataset index building on rank 0 - all subsequent iterations ran at ~32 seconds."
)
...
steps = [
    ("HuggingFace conversion",
     "Run convert_checkpoint.sbatch (job 18602184) to convert iter_0001000 to "
     "LlamaForCausalLM format. Manually add rope_scaling to config.json:\n"
     '   {"factor": 16.0, "original_max_position_embeddings": 2048, "type": "yarn"}, '
     '"rope_theta": 10000'),
    ("Long-context evaluation",
     "Run RULER, Needle-in-a-Haystack, and LongBench on the converted checkpoint "
     "to confirm 32K context capability on downstream tasks."),
    ("Scale data",
     "Add remaining 27 languages via tokenize_tiers.sbatch (ro, uk, sr, hu, pl, ...) "
     "and retrain with a broader multilingual mix."),
    ("LongRoPE search",
     "Run longrope_search_tokenize.sbatch + longrope_search.sbatch for multilingual "
     "RoPE factors; retrain with longrope_multilingual.sbatch if results improve on eval."),
]
for i, (title, desc) in enumerate(steps):
    pdf.set_font("Helvetica", "B", 9.5)
    pdf.set_fill_color(240, 244, 255) if i % 2 == 0 else pdf.set_fill_color(255, 255, 255)
    pdf.cell(0, 7, f"  {i+1}. {title}", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_font("Helvetica", "", 9)
...
# ── Save ──────────────────────────────────────────────────────────────────────
out = "/Users/birgermoell/AI-Sweden/openeuro-longctx-datamix/docs/training_report_job18536300.pdf"
pdf.output(out)
print(f"Written: {out}")
